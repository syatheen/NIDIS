...
GlobSnow3_RefFileName = '/discover/nobackup/projects/nca/jacaraba/NIDIS_Data/Indicator_113/weekly_resolution/RefArrays/ClimGrid_GlobSnow3_'+format(GlobSnow3_Ref_BeginDateVecList[0],'04')+format(GlobSnow3_Ref_BeginDateVecList[1],'02')+format(GlobSnow3_Ref_BeginDateVecList[2],'02')+'To'+format(GlobSnow3_Ref_EndDateVecList[0],'04')+format(GlobSnow3_Ref_EndDateVecList[1],'02')+format(GlobSnow3_Ref_EndDateVecList[2],'02')+'.npz'
...
for WhichYear in range(GlobSnow3_InfoFiles_BeginYYYYMMDDVecList[0], GlobSnow3_InfoFiles_EndYYYYMMDDVecList[0]+1):
  ...
  GlobSnow3_ThisYearZeroFill_BeginDate = date(WhichYear, AnyYearZeroFill_BeginMonth, AnyYearZeroFill_BeginDoM)
  GlobSnow3_ThisYearZeroFill_EndDate = date(WhichYear, AnyYearZeroFill_EndMonth, AnyYearZeroFill_EndDoM)
  ...
  for WhichMonth in range(BeginMonth, EndMonth+1):
    ...
    for WhichDoM in range(BeginDoM, EndDoM+1):
      ...
      IfFileExists = os.path.exists(GlobSnow3_InfoFilesDir + 'GlobSnow3_' + format(WhichYear, '04') + format(WhichMonth, '02') + format(WhichDoM, '02') + '_ClimGrid.npz')

      if IfFileExists:

        ThisYYYYMMDD_ClimDivs_Info = np.load(GlobSnow3_InfoFilesDir + 'GlobSnow3_' + format(WhichYear, '04') + format(WhichMonth, '02') + format(WhichDoM, '02') + '_ClimGrid.npz')
  
        ThisYYYYMMDD_Of_InfoArrayForPrcntl = ThisYYYYMMDD_ClimDivs_Info['YYYYMMDD_Of_InfoArray']
        ThisYYYYMMDD_InfoArrayForPrcntl = ThisYYYYMMDD_ClimDivs_Info['InfoArray'] 
        ...
        ThisYYYYMMDD_InfoArrayForPrcntl = np.empty((1, GlobSnow3_InfoArray.shape[1]))
        GlobSnow3_ThisYYYYMMDD_Date = date(WhichYear, WhichMonth, WhichDoM)
        if ( (GlobSnow3_ThisYYYYMMDD_Date >= GlobSnow3_ThisYearZeroFill_BeginDate) and 
             (GlobSnow3_ThisYYYYMMDD_Date <= GlobSnow3_ThisYearZeroFill_EndDate) ): 
          ThisYYYYMMDD_InfoArrayForPrcntl[:] = 0.0
        else:
          ThisYYYYMMDD_InfoArrayForPrcntl[:] = np.NaN
        #end of if ( (GlobSnow3_ThisYYYYMMDD_Date >= GlobSnow3_ThisYearZeroFill_BeginDate) and...

      #end of if IfFileExists 

      if ( (WhichYear == GlobSnow3_InfoFiles_BeginYYYYMMDDVecList[0]) and
           (WhichMonth == GlobSnow3_InfoFiles_BeginYYYYMMDDVecList[1]) and
           (WhichDoM == GlobSnow3_InfoFiles_BeginYYYYMMDDVecList[2]) ):
