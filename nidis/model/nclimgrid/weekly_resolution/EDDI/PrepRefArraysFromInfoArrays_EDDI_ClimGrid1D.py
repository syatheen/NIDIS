...
EDDI_InfoFilesDir = '/discover/nobackup/projects/nca/syatheen/EDDI_Npzs/'
...
EDDI_InfoFiles_EndYear = 2020 # Ends [2022, 4, 15]
EDDI_InfoFiles_EndMonth = 12 
EDDI_InfoFiles_EndDayOfMonth = 31 

EDDI_Ref_BeginDateVecList = [1980, 1, 8]  # EDDI beginning year, month, day of month, this is also a Tuesday
EDDI_Ref_EndDateVecList = [2022, 4, 12]  # EDDI ending year, month, day of month, this is also a Tuesday

EDDI_RefFileName = 'RefArrays/ClimGrid1D_EDDI_'+EDDI_WhichPastPeriod+'_'+format(EDDI_Ref_BeginDateVecList[0],'04')+format(EDDI_Ref_BeginDateVecList[1],'02')+format(EDDI_Ref_BeginDateVecList[2],'02')+'To'+format(EDDI_Ref_EndDateVecList[0],'04')+format(EDDI_Ref_EndDateVecList[1],'02')+format(EDDI_Ref_EndDateVecList[2],'02')+'.npz'
...
for ThisYear in range(EDDI_InfoFiles_BeginYear, EDDI_InfoFiles_EndYear+1, 1):
  if ThisYear == 2022:
    ThisYear_EDDI_Info = np.load(EDDI_InfoFilesDir + format(EDDI_InfoFiles_EndYear, '04') + '0101To' + format(EDDI_InfoFiles_EndYear, '04') + format(EDDI_InfoFiles_EndMonth, '02') + format(EDDI_InfoFiles_EndDayOfMonth, '02')  + '_' + EDDI_WhichPastPeriod + '.npz')
  else:
    ThisYear_EDDI_Info = np.load(EDDI_InfoFilesDir + format(ThisYear, '04') + '0101To' + format(ThisYear, '04') + '1231_'+ EDDI_WhichPastPeriod + '.npz')
...
print("np.amin(np.isnan(EDDI_RefArray).sum(axis=1)) is ",np.amin(np.isnan(EDDI_RefArray).sum(axis=1)))
print("np.amax(np.isnan(EDDI_RefArray).sum(axis=1)) is ",np.amax(np.isnan(EDDI_RefArray).sum(axis=1)))
