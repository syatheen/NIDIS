...
#         on Nov 13, 2024
# NOTE: This is a non-multiprocessing & original style copy of InfoArrays_gdalWarp_ClimGrid1D_DailyCollToMonthly_Indicators_97.py 
...
path_to_save_data = '/discover/nobackup/projects/nca/syatheen/ESA_CCI_Npzs'
os.makedirs(path_to_save_data, exist_ok=True)
  
ArrayFileName = os.path.join(
    path_to_save_data, 'ESA_CCI_' + format(ArgYearInt,'04') + format(ArgMonthInt,'02') + '_ClimGrid1D.npz')
  
SourceFileBasePath = '/discover/nobackup/projects/nca/syatheen/ESA_CCI'
SourceFileBasePathWrite = '/discover/nobackup/projects/nca/syatheen/ESA_CCI'
...
for WhichDayInMonth in range(1, NumDaysInMonth+1):
    ...
    SourceFile = os.path.join(SourceFileBasePath, 'ESA_CCI_' + format(ArgYearInt,'04') + format(ArgMonthInt,'02') + format(WhichDayInMonth,'02') + '.tif')

    IfTifFileExists = os.path.exists(SourceFile)
    if IfTifFileExists:

        BaseFileName =  'ESA_CCI' + format(ArgYearInt,'04') + format(ArgMonthInt,'02') + format(WhichDayInMonth,'02') 
...
Idxs =  np.where(RefArrayForPrcntl < ESA_CCI_LowerLimit)
