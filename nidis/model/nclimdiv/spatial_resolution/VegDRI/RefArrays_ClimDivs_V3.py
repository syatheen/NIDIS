...
ArrayFileName = 'RefArrsWeekly/'+format(ThisYear,'04')+format(ThisMonth,'02')+format(ThisDoM,'02')+'.npz'
...
FileNames_VegDRI=glob.glob(SourceFilePath+format(ThisYear,'04')+'/vegdri_emodis_week'+date(ThisYear, ThisMonth, ThisDoM).strftime("%U").lstrip('0')+'_'+format(ThisMonth,'02')+format(ThisDoM,'02')+format(ThisYear%100,'02')+'.tif')

if (len(FileNames_VegDRI) == 1):

  BaseFileNameWithPath = SourceFilePath+format(ThisYear,'04')+'/vegdri_emodis_week'+date(ThisYear, ThisMonth, ThisDoM).strftime("%U").lstrip('0')+'_'+format(ThisMonth,'02')+format(ThisDoM,'02')+format(ThisYear%100,'02')
  BaseFileName = 'vegdri_emodis_week'+date(ThisYear, ThisMonth, ThisDoM).strftime("%U").lstrip('0')+'_'+format(ThisMonth,'02')+format(ThisDoM,'02')+format(ThisYear%100,'02')
  ...
  if IfTifFileExists:
    ...
    Valid_GridPoints_XYZ_df = GridPoints_XYZ_df.loc[(GridPoints_XYZ_df.Z >= VegDRI_LowerLimit) & (GridPoints_XYZ_df.Z <= VegDRI_UpperLimit)]
