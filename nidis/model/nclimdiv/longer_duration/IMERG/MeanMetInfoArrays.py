...
OutFilePath = 'InfoArrsDailyMean/ClimDivs/'
...
for WhichYear in range(BeginYYYYMM[0], EndYYYYMM[0]+1):
  ...
  for WhichMonth in range(BeginMonth, EndMonth+1):
    ...
    InfoArrayFile_Info = np.load('/discover/nobackup/projects/nca/syatheen/IMERG_Npzs/daily_Final_V06/IMERG_' + format(WhichYear,'04') + format(WhichMonth,'02') + '_ClimDivs.npz')
...
if (np.isnan(InfoArrayForPrcntl).sum() != 0):
  ...
  BeginYear = int(round(YYYYMMDD_Of_InfoArrayForPrcntl[0] // 10000))
  BeginMonth = int(round((YYYYMMDD_Of_InfoArrayForPrcntl[0] - 10000 * BeginYear) // 100))
  BeginDayOfMonth = int(round(YYYYMMDD_Of_InfoArrayForPrcntl[0] % 100))
  EndYear = int(round(YYYYMMDD_Of_InfoArrayForPrcntl[-1] // 10000))
  EndMonth = int(round((YYYYMMDD_Of_InfoArrayForPrcntl[-1] - 10000 * EndYear) // 100))
  EndDayOfMonth = int(round(YYYYMMDD_Of_InfoArrayForPrcntl[-1] % 100))
