...
def run_calc(parameters):
    ...
    CalcFI1X1_ClimGrid1D_V2b_NDFeat_NewSeas_Main(
        ...
        NumInpLayers=int(round(float(parameters[6]))), #int(round(float(sys.argv[7]))), # 113 for all input channels (SY: temporarily using -15, this needs to be changed), 0 for All CPC Blend input channels, -10 for remotely-sensed, -11 for modeled, -12 for modeled+PrecipObs, -50 for all NDMC Blend input channels
